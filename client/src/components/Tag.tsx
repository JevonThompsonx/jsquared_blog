...
import { Article, Post, Tag as TagType } from "../../../shared/src/types";
import SEO from "./SEO";
...
const NoResults: FC<{ tagName: string }> = ({ tagName }) => (
  <div className="col-span-full rounded-[2rem] border border-[var(--border)] bg-[var(--card-bg)] p-12 text-center shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
    <p className="text-xs uppercase tracking-[0.35em] text-[var(--primary)] mb-3">Tag Empty</p>
    <h3 className="text-2xl font-semibold text-[var(--text-primary)] mb-3">No adventures tagged {tagName}</h3>
    <p className="mx-auto max-w-xl text-sm text-[var(--text-secondary)]">
      That topic does not have any public journal entries yet. Try a related tag or head back to the homepage.
    </p>
  </div>
);
...
const TagPage: FC = () => {
  ...
  const fetchPosts = useCallback(async (currentOffset: number, tagSlug: string, isInitial: boolean = false) => {
    ...
    try {
      ...
      const response = await fetch(apiPath(`/api/tags/${tagSlug}/posts?${params}`));
      ...
      const publishedPosts = (data.posts || []).filter((post: Post) => post.status === "published");

      if (isInitial) {
        setTag(data.tag);
        setAllPosts(publishedPosts);
        setTotalCount(data.total || publishedPosts.length);
      } else {
        setAllPosts((prev) => [...prev, ...publishedPosts]);
      }

      setHasMore(publishedPosts.length === POSTS_PER_PAGE && data.hasMore !== false);
    } catch (error) {
  ...
  }, [fetchPosts, hasMore, isLoading, isLoadingMore, offset, slug]);

  const displayedArticles = useMemo(() => assignLayoutAndGridClass(allPosts), [allPosts]);

  if (notFound) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--background)] pt-24 pb-12">
        <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--card-bg)] p-12 text-center shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
          <h1 className="text-3xl font-semibold text-[var(--text-primary)] mb-3">Tag Not Found</h1>
          <p className="text-[var(--text-secondary)] mb-6">The tag “{slug}” does not exist.</p>
          <Link to="/" className="inline-flex items-center gap-2 rounded-full bg-[var(--primary)] px-5 py-3 text-sm font-medium text-white">
            Back to Home
          </Link>
        </div>
      </div>
  ...
  const heading = tag?.name || slug || "Tag";

  return (
    <>
      <SEO
        title={`${heading} Posts`}
        description={`Explore all adventures tagged with ${heading} on J²Adventures.`}
        type="website"
      />
      ...
      <div className="min-h-screen bg-[var(--background)] pt-24 pb-16">
        <section className="container mx-auto px-4 sm:px-6 lg:px-8 mb-10">
          ...
          <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--card-bg)] p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)] md:p-10">
            <p className="text-xs uppercase tracking-[0.35em] text-[var(--primary)] mb-4">Tagged Archive</p>
            <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
              <div className="space-y-4">
                <h1 className="text-4xl md:text-5xl font-semibold leading-tight text-[var(--text-primary)]">#{heading}</h1>
                <p className="max-w-2xl text-base md:text-lg leading-8 text-[var(--text-secondary)]">
                  A cozy little collection of adventures gathered around one theme, one mood, or one repeat obsession.
                </p>
              </div>
        ...
        <main className="container mx-auto px-4 sm:px-6 lg:px-8">
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-8">
              <NoResults tagName={heading} />
            </div>
          )}
        </main>

        {slug && !isLoading && <RelatedTags currentTagSlug={slug} limit={8} />}
