...
// .wrangler/tmp/bundle-87SLpB/checked-fetch.js
...
// .wrangler/tmp/bundle-87SLpB/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-87SLpB/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var authMiddleware = createMiddleware(async (c, next) => {
  ...
  const supabase = createClient(
    c.env.SUPABASE_URL,
    // Use c.env for Cloudflare Workers
...
// src/index.ts
async function uploadImageToCloudflareImages(imageBuffer, imageName, contentType, env) {
  ...
  if (!response.ok) {
    ...
    console.error("Cloudflare Images upload error (full response):", JSON.stringify(errorData, null, 2));
    throw new Error(`Cloudflare Images upload failed: ${errorData.errors?.[0]?.message || response.statusText}`);
  }
  const result = await response.json();
  return result.result.id;
}
__name(uploadImageToCloudflareImages, "uploadImageToCloudflareImages");
async function deleteImageFromCloudflareImages(imageId, env) {
...
__name(deleteImageFromCloudflareImages, "deleteImageFromCloudflareImages");
async function reassignAllPostLayouts(supabase, layoutPatterns) {
  const { data: allPosts, error: fetchAllError } = await supabase.from("posts").select("*").order("created_at", { ascending: false });
  if (fetchAllError) {
  ...
  const updates = allPosts.map((post2, index2) => {
    const patternIndex = index2 % layoutPatterns.length;
    const selectedPattern = layoutPatterns[patternIndex];
    let assignedType = selectedPattern.type;
    ...
    return {
      ...post2,
      // Include all existing properties of the post
      type: assignedType,
...
__name(reassignAllPostLayouts, "reassignAllPostLayouts");
var app = new Hono2();
app.use("*", logger());
app.get("/", (c) => {
...
app.post("/api/posts", authMiddleware, async (c) => {
  ...
  if (imageFile) {
    try {
      ...
      console.error("Image optimization/upload process failed:", error2);
      return c.json({ error: `Image optimization/upload failed: ${error2.message}` }, 500);
    }
  } else if (imageUrl) {
...
app.put("/api/posts/:id", authMiddleware, async (c) => {
  ...
  const description = body.description || null;
  const category = body.category || null;
  const grid_class = body.grid_class || null;
  const postType = body.type || null;
  const imageFile = body.image;
  const imageUrl = body.image_url;
  if (!title) {
    return c.json({ error: "Title is required" }, 400);
  }
  const { data: existingPost, error: fetchError } = await supabase.from("posts").select("author_id, image_url, type, grid_class").eq("id", id).single();
  if (fetchError || !existingPost) {
    return c.json({ error: "Post not found or unauthorized" }, 404);
  }
  if (existingPost.author_id !== user.id && existingPost.author_id !== null && // If author_id is not null, it must match user.id
  user.role !== "admin") {
    return c.json({ error: "Unauthorized to edit this post" }, 403);
  }
  let finalImageUrl = existingPost.image_url;
  if (imageFile) {
    try {
      if (existingPost.image_url && existingPost.image_url.includes("imagedelivery.net")) {
        const oldImageId = existingPost.image_url.split("/")[4];
        if (oldImageId) {
      ...
      console.error("Image optimization/upload process failed (PUT):", error2);
      return c.json({ error: `Image optimization/upload failed: ${error2.message}` }, 500);
    }
  ...
  const { data, error } = await supabase.from("posts").update({
    ...
    // Use the determined image URL
    category,
    grid_class: grid_class !== null ? grid_class : existingPost.grid_class,
    // Use new grid_class if provided, else existing
    type: postType !== null ? postType : existingPost.type
    // Use new type if provided, else existing
  }).eq("id", id).select();
  if (error) {
