          ...
          <div className="flex h-full max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-[var(--background)] shadow-2xl border border-[var(--border)]">
            ...
            <div className="flex flex-1 overflow-hidden">
              <div className="w-1/3 overflow-y-auto border-r border-[var(--border)] bg-[var(--card-bg)] p-4 space-y-2">
                {isLoading && !revisions.length ? <p className="text-sm text-[var(--text-secondary)]">Loading...</p> : null}
                {revisions.map(rev => (
                  <button
                    key={rev.id}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="mt-4 rounded-lg bg-[var(--card-bg)] p-4 border border-[var(--border)]">
                      <pre className="text-xs whitespace-pre-wrap font-mono text-[var(--text-secondary)]">
                        {JSON.stringify(JSON.parse(selectedRevision.contentJson), null, 2)}
                      </pre>
                    </div>
                  </div>
                ) : (
                  <p className="text-[var(--text-secondary)]">Select a revision to view details.</p>
                )}
              </div>
            </div>
          </div>
