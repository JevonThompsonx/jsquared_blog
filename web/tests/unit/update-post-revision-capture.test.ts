...
const EXISTING_POST = {
  ...
  featuredImageId: "media-existing",
...
describe("updateAdminPostAction — revision capture", () => {
  ...
  it("calls createPostRevision with the pre-update content after a successful save", async () => {
    ...
    expect(vi.mocked(createPostRevision)).toHaveBeenCalledWith({
      ...
      contentJson: EXISTING_POST.contentJson,
      excerpt: EXISTING_POST.excerpt,
      layoutType: EXISTING_POST.layoutType,
      categoryId: EXISTING_POST.categoryId,
      featuredImageId: EXISTING_POST.featuredImageId,
      locationName: EXISTING_POST.locationName,
      locationLat: EXISTING_POST.locationLat,
      locationLng: EXISTING_POST.locationLng,
      locationZoom: EXISTING_POST.locationZoom,
